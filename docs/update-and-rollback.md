...
docker compose run --rm --no-deps gateway corpusgate-admin doctor
docker compose up -d
./corpusgate status
./corpusgate list-documents --limit 1
```
...
For semantic deployments, use both Compose files for build/start and run `./corpusgate reindex
